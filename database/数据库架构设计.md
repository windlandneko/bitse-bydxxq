...
|---|---|---|
| `id` | bigint | PK |
| `phone` | varchar(64) | NOT NULL，唯一；按文本保存，格式由服务层按地区规则处理 |
| `nickname` | varchar(64) | NOT NULL；新用户默认由服务层生成“用户”+手机号后四位 |
| `avatar_path` | varchar(512) | 可空，仅保存本地/对象存储引用，不存二进制图片 |
| `wallet_balance` | numeric(12,2) | NOT NULL DEFAULT 0；允许用负值直接表示欠费 |
...
手机号是登录标识，不记录短信验证码；免密登录由服务层按手机号查找或创建用户，昵称默认生成“用户”+手机号后四位。
...
| `balance_before`,`balance_after` | numeric(12,2) | NOT NULL；余额可为负，且 `balance_after = balance_before + amount` |
| `order_id` | bigint | FK -> `orders.id`；充电扣款和退款流水必填，充值/调账可空 |
| `idempotency_key` | varchar(80) | NOT NULL UNIQUE，用于防重复扣款/充值 |
| `remark` | varchar(255) | 可空 |
| `created_at` | TEXT | ISO-8601 UTC，NOT NULL |

`users.wallet_balance` 是快速读取的当前余额，`wallet_transactions` 是不可变流水；两者必须在同一个数据库事务中更新。一次充电扣款可以使余额变为负值并直接展示为欠费；当扣款前余额小于等于 0 时，服务层应阻止后续扣款。
...
保存模型输出：`station_id`、`forecast_time`、`horizon_start`、`horizon_end`、`predicted_load_kw`、`predicted_available_chargers`、`is_peak`、`model_version`、`actual_load_kw`（回填评估）和 `generated_at`。唯一键建议为 `(station_id, horizon_start, horizon_end, model_version)`，支持重复训练而不覆盖不同模型版本。
...
- `users.phone`、`admins.username`、各业务编号全部唯一；手机号按文本保存，具体格式校验由服务层负责。
- SQLite 使用部分唯一索引：同一用户最多一个 `reservations.status in ('active','started')`，同一电桩最多一个有效预约；同一用户和同一电桩最多一个 `charging_sessions.status in ('preparing','charging','interrupted','settling')`。
- `orders.payable_amount >= 0`、余额流水前后值符合金额加减、结束时间不早于开始时间；订单状态与 `paid_amount/settled_at` 的组合由 CHECK 强制一致。
- `charging_sessions` 的预约、用户、电桩和站点关联，以及 `orders` 与会话的关联由 SQLite 触发器强制一致；取消/过期预约不能创建新的充电会话，订单不能关联仍在准备或充电中的会话。
...
3. 若扣款前余额小于等于 0，阻止本次后续扣款并返回业务错误；若余额大于 0，则允许本次完整扣款，即使扣款后余额变为负值，并在用户端直接显示欠费。
4. 更新用户余额，插入一条负金额 `wallet_transactions`，订单置 `paid`，会话置 `settled`，电桩恢复 `idle` 并累加统计。
...
初始化数据应包含：1 个管理员账号、5 个电站、每站 6-12 个混合类型电桩并包含 1-2 个故障桩、每站至少 1 条当前生效价格，以及覆盖充值、已结算、负余额欠费和异常中断的至少 30 天历史订单。种子数据必须使用固定业务编号，重复执行不产生重复记录。
...
3. 冻结用户无法预约或启动充电；
4. 扣款后余额不足时余额可为负，且余额小于等于 0 时后续扣款被阻止；
